...
export default function PaymentUpdateFeePaymentModal({
  openDialog,
  selectedFeePayment,
}) {
  ...
  const requestParams = [
    {
      name: "paymentStatus",
      value: selectedFeePayment?.paymentStatus ?? "",
      type: "Enum",
    },

    {
      name: "paymentDate",
      value: selectedFeePayment?.paymentDate ?? "",
      type: "Date",
    },

    { name: "note", value: selectedFeePayment?.note ?? "", type: "Text" },
  ];

  const defaultValues = {
    paymentStatus: selectedFeePayment?.paymentStatus ?? "",

    paymentDate: selectedFeePayment?.paymentDate ?? "",

    note: selectedFeePayment?.note ?? "",
  };
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      if (selectedFeePayment?.id) {
        const response = await paymentAxios.patch(
          paymentEndpoints.feePayment.updateFeePayment.replace(
            ":feePaymentId",
            selectedFeePayment?.id,
          ),
          data,
        );
        setError(null);
  ...
  if (!selectedFeePayment) return null;

  return (
    <Dialog open={openDialog.value} maxWidth="md">
      <DialogTitle>Update FeePayment</DialogTitle>
