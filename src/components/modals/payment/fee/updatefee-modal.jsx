...
const FeeSchema = zod.object({
  ...
  statusUpdateDate: zod.string().nullable(),

  note: zod.string().nullable(),
});

export default function PaymentUpdateFeeModal({ openDialog, selectedFee }) {
  const [error, setError] = useState(null);

  const theme = useTheme();

  const requestParams = [
    { name: "status", value: selectedFee?.status ?? "", type: "Enum" },

    {
      name: "statusUpdateDate",
      value: selectedFee?.statusUpdateDate ?? "",
      type: "Date",
    },

    { name: "note", value: selectedFee?.note ?? "", type: "Text" },
  ];

  const defaultValues = {
    status: selectedFee?.status ?? "",

    statusUpdateDate: selectedFee?.statusUpdateDate ?? "",
    ...
    try {
      if (selectedFee?.id) {
        const response = await paymentAxios.patch(
          paymentEndpoints.fee.updateFee.replace(":feeId", selectedFee?.id),
          data,
        );
        setError(null);
        reset();
