...
const requestParams = [
  ...
  { name: "socialCode", value: "", type: "String" },

  { name: "password", value: "", type: "String" },

  { name: "name", value: "", type: "String" },

  { name: "surname", value: "", type: "String" },

  { name: "email", value: "", type: "String" },

  { name: "mobile", value: "", type: "String" },
];

const UserSchema = zod.object({
  avatar: zod.string().nullable(),

  socialCode: zod.string().nullable(),
...
export default function AuthRegisterUserApiPage() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const response = await authAxios.post(
        authEndpoints.user.registerUser,
        data,
      );
      setError(null);
      setCreatedUser(null);
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(createdUser || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={createdUser} error={error} />
            ) : (
              <JsonResponse content={createdUser || error} />
            )}
          </Box>
        </Box>
      )}
