...
export default function AuthCreateGroupApiPage() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const response = await authAxios.post(
        authEndpoints.userGroup.createGroup,
        data,
      );
      setError(null);
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(createdGroup || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={createdGroup} error={error} />
            ) : (
              <JsonResponse content={createdGroup || error} />
            )}
          </Box>
        </Box>
      )}
