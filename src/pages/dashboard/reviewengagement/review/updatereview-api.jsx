...
export default function ReviewEngagementUpdateReviewApiPage() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setUpdatedReview(response.data.review);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(updatedReview || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={updatedReview} error={error} />
            ) : (
              <JsonResponse content={updatedReview || error} />
            )}
          </Box>
        </Box>
      )}
