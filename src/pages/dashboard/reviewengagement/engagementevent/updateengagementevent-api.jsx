...
const requestParams = [{ name: "details", value: "", type: "Object" }];

const EngagementeventSchema = zod.object({
  details: zod.string().nullable(),
});

export default function ReviewEngagementUpdateEngagementEventApiPage() {
  const [view, setView] = useState("Table");
  const [updatedEngagementevent, setUpdatedEngagementevent] = useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setError(null);
      setUpdatedEngagementevent(null);
      reset();
      console.info("RESPONSE", response);
      setUpdatedEngagementevent(response.data.engagementEvent);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(updatedEngagementevent || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={updatedEngagementevent} error={error} />
            ) : (
              <JsonResponse content={updatedEngagementevent || error} />
            )}
          </Box>
        </Box>
      )}
