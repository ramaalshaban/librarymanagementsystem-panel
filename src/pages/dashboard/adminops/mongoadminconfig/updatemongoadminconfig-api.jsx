...
export default function AdminOpsUpdateMongoAdminConfigApiPage() {
  const [view, setView] = useState("Table");
  const [updatedMongoadminconfig, setUpdatedMongoadminconfig] = useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setError(null);
      setUpdatedMongoadminconfig(null);
      reset();
      console.info("RESPONSE", response);
      setUpdatedMongoadminconfig(response.data.mongoAdminConfig);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(updatedMongoadminconfig || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={updatedMongoadminconfig} error={error} />
            ) : (
              <JsonResponse content={updatedMongoadminconfig || error} />
            )}
          </Box>
        </Box>
      )}
