...
const SystembackupauditSchema = zod.object({
  ...
  config: zod.string().min(1, { message: "config is required" }),
...
export default function AdminOpsCreateSystemBackupAuditApiPage() {
  const [view, setView] = useState("Table");
  const [createdSystembackupaudit, setCreatedSystembackupaudit] =
    useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const response = await adminOpsAxios.post(
        adminOpsEndpoints.systemBackupAudit.createSystemBackupAudit,
        data,
      );
      setError(null);
      setCreatedSystembackupaudit(null);
      reset();
      console.info("RESPONSE", response);
      setCreatedSystembackupaudit(response.data.systembackupaudit);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
  ...
  return (
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      <Divider />
      {(createdSystembackupaudit || error) && (
        <Box paddingTop="2rem">
          <Box
            display="flex"
            flexDirection="row"
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={createdSystembackupaudit} error={error} />
            ) : (
              <JsonResponse content={createdSystembackupaudit || error} />
            )}
          </Box>
        </Box>
      )}
