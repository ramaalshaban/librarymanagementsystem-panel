...
export default function AdminOpsUpdateIssueEscalationApiPage() {
  const [view, setView] = useState("Table");
  const [updatedIssueescalation, setUpdatedIssueescalation] = useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setError(null);
      setUpdatedIssueescalation(null);
      reset();
      console.info("RESPONSE", response);
      setUpdatedIssueescalation(response.data.issueEscalation);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(updatedIssueescalation || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={updatedIssueescalation} error={error} />
            ) : (
              <JsonResponse content={updatedIssueescalation || error} />
            )}
          </Box>
        </Box>
      )}
