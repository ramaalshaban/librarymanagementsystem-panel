...
export default function AdminOpsUpdateBranchPurchaseOrderApiPage() {
  const [view, setView] = useState("Table");
  const [updatedBranchpurchaseorder, setUpdatedBranchpurchaseorder] =
    useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setError(null);
      setUpdatedBranchpurchaseorder(null);
      reset();
      console.info("RESPONSE", response);
      setUpdatedBranchpurchaseorder(response.data.branchPurchaseOrder);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
  ...
  return (
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      <Divider />
      {(updatedBranchpurchaseorder || error) && (
        <Box paddingTop="2rem">
          <Box
            display="flex"
            flexDirection="row"
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse
                content={updatedBranchpurchaseorder}
                error={error}
              />
            ) : (
              <JsonResponse content={updatedBranchpurchaseorder || error} />
            )}
          </Box>
        </Box>
      )}
