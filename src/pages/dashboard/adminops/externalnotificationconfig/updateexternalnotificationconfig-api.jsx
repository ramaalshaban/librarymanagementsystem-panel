...
export default function AdminOpsUpdateExternalNotificationConfigApiPage() {
  const [view, setView] = useState("Table");
  const [
    updatedExternalnotificationconfig,
    setUpdatedExternalnotificationconfig,
  ] = useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setError(null);
      setUpdatedExternalnotificationconfig(null);
      reset();
  ...
  return (
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      <Divider />
      {(updatedExternalnotificationconfig || error) && (
        <Box paddingTop="2rem">
          <Box
            display="flex"
            flexDirection="row"
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse
                content={updatedExternalnotificationconfig}
                error={error}
              />
            ) : (
              <JsonResponse
                content={updatedExternalnotificationconfig || error}
              />
            )}
          </Box>
        </Box>
