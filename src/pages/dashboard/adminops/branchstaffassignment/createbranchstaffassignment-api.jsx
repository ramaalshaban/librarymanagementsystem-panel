...
export default function AdminOpsCreateBranchStaffAssignmentApiPage() {
  const [view, setView] = useState("Table");
  const [createdBranchstaffassignment, setCreatedBranchstaffassignment] =
    useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const response = await adminOpsAxios.post(
        adminOpsEndpoints.branchStaffAssignment.createBranchStaffAssignment,
        data,
      );
      setError(null);
      setCreatedBranchstaffassignment(null);
      reset();
      console.info("RESPONSE", response);
      setCreatedBranchstaffassignment(response.data.branchstaffassignment);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
  ...
  return (
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      <Divider />
      {(createdBranchstaffassignment || error) && (
        <Box paddingTop="2rem">
          <Box
            display="flex"
            flexDirection="row"
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse
                content={createdBranchstaffassignment}
                error={error}
              />
            ) : (
              <JsonResponse content={createdBranchstaffassignment || error} />
            )}
          </Box>
        </Box>
      )}
