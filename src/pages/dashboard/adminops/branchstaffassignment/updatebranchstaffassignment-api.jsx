...
export default function AdminOpsUpdateBranchStaffAssignmentApiPage() {
  const [view, setView] = useState("Table");
  const [updatedBranchstaffassignment, setUpdatedBranchstaffassignment] =
    useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setError(null);
      setUpdatedBranchstaffassignment(null);
      reset();
      console.info("RESPONSE", response);
      setUpdatedBranchstaffassignment(response.data.branchStaffAssignment);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
  ...
  return (
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      <Divider />
      {(updatedBranchstaffassignment || error) && (
        <Box paddingTop="2rem">
          <Box
            display="flex"
            flexDirection="row"
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse
                content={updatedBranchstaffassignment}
                error={error}
              />
            ) : (
              <JsonResponse content={updatedBranchstaffassignment || error} />
            )}
          </Box>
        </Box>
      )}
