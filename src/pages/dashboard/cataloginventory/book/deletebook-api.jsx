...
export default function CatalogInventoryDeleteBookApiPage() {
  ...
  const handleDeleteBook = async () => {
    try {
      ...
      const response = await catalogInventoryAxios.delete(
        catalogInventoryEndpoints.book.deleteBook.replace(
          ":bookId",
          inputBookId,
        ),
      );
      ...
      setDeletedBook(response.data.book);
    ...
    <Box>
      ...
      {!bookLoading && (error || deletedBook) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={deletedBook} error={error} />
            ) : (
              <JsonResponse content={deletedBook || error} />
            )}
          </Box>
        </Box>
      )}
