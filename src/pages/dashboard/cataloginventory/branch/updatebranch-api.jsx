...
const requestParams = [
  ...
  { name: "address", value: "", type: "Object" },

  { name: "geoLocation", value: "", type: "Object" },

  { name: "contactEmail", value: "", type: "String" },
];
...
export default function CatalogInventoryUpdateBranchApiPage() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setUpdatedBranch(response.data.branch);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(updatedBranch || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={updatedBranch} error={error} />
            ) : (
              <JsonResponse content={updatedBranch || error} />
            )}
          </Box>
        </Box>
      )}
