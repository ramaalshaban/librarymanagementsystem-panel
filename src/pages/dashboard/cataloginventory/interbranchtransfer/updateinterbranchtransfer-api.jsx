...
export default function CatalogInventoryUpdateInterBranchTransferApiPage() {
  const [view, setView] = useState("Table");
  const [updatedInterbranchtransfer, setUpdatedInterbranchtransfer] =
    useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setError(null);
      setUpdatedInterbranchtransfer(null);
      reset();
      console.info("RESPONSE", response);
      setUpdatedInterbranchtransfer(response.data.interBranchTransfer);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
  ...
  return (
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      <Divider />
      {(updatedInterbranchtransfer || error) && (
        <Box paddingTop="2rem">
          <Box
            display="flex"
            flexDirection="row"
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse
                content={updatedInterbranchtransfer}
                error={error}
              />
            ) : (
              <JsonResponse content={updatedInterbranchtransfer || error} />
            )}
          </Box>
        </Box>
      )}
