...
export default function CatalogInventoryCreateBranchInventoryApiPage() {
  const [view, setView] = useState("Table");
  const [createdBranchinventory, setCreatedBranchinventory] = useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setError(null);
      setCreatedBranchinventory(null);
      reset();
      console.info("RESPONSE", response);
      setCreatedBranchinventory(response.data.branchinventory);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(createdBranchinventory || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={createdBranchinventory} error={error} />
            ) : (
              <JsonResponse content={createdBranchinventory || error} />
            )}
          </Box>
        </Box>
      )}
