...
export default function CatalogInventoryUpdatePurchaseOrderApiPage() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setUpdatedPurchaseorder(response.data.purchaseOrder);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(updatedPurchaseorder || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={updatedPurchaseorder} error={error} />
            ) : (
              <JsonResponse content={updatedPurchaseorder || error} />
            )}
          </Box>
        </Box>
      )}
