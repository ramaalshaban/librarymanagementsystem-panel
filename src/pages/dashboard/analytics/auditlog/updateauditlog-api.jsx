...
export default function AnalyticsUpdateAuditLogApiPage() {
  const [view, setView] = useState("Table");
  const [updatedAuditlog, setUpdatedAuditlog] = useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setError(null);
      setUpdatedAuditlog(null);
      reset();
      console.info("RESPONSE", response);
      setUpdatedAuditlog(response.data.auditLog);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(updatedAuditlog || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={updatedAuditlog} error={error} />
            ) : (
              <JsonResponse content={updatedAuditlog || error} />
            )}
          </Box>
        </Box>
      )}
