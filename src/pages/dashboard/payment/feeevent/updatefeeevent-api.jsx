...
export default function PaymentUpdateFeeEventApiPage() {
  const [view, setView] = useState("Table");
  const [updatedFeeevent, setUpdatedFeeevent] = useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const response = await paymentAxios.patch(
        paymentEndpoints.feeEvent.updateFeeEvent.replace(
          ":feeEventId",
          inputFeeEventId,
        ),
        data,
      ...
      setError(null);
      setUpdatedFeeevent(null);
      reset();
      console.info("RESPONSE", response);
      setUpdatedFeeevent(response.data.feeEvent);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(updatedFeeevent || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={updatedFeeevent} error={error} />
            ) : (
              <JsonResponse content={updatedFeeevent || error} />
            )}
          </Box>
        </Box>
      )}
