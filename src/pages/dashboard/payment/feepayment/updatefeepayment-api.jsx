...
export default function PaymentUpdateFeePaymentApiPage() {
  const [view, setView] = useState("Table");
  const [updatedFeepayment, setUpdatedFeepayment] = useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setError(null);
      setUpdatedFeepayment(null);
      reset();
      console.info("RESPONSE", response);
      setUpdatedFeepayment(response.data.feePayment);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(updatedFeepayment || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={updatedFeepayment} error={error} />
            ) : (
              <JsonResponse content={updatedFeepayment || error} />
            )}
          </Box>
        </Box>
      )}
