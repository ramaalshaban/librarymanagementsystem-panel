...
export default function PaymentCreatePaymentApiPage() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const response = await paymentAxios.post(
        paymentEndpoints.feePayment.createPayment,
        data,
      );
      setError(null);
      ...
      setCreatedPayment(response.data.payment);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(createdPayment || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={createdPayment} error={error} />
            ) : (
              <JsonResponse content={createdPayment || error} />
            )}
          </Box>
        </Box>
      )}
