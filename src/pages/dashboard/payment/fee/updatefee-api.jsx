...
export default function PaymentUpdateFeeApiPage() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const response = await paymentAxios.patch(
        paymentEndpoints.fee.updateFee.replace(":feeId", inputFeeId),
        data,
      );
      setError(null);
      setUpdatedFee(null);
      ...
      setUpdatedFee(response.data.fee);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(updatedFee || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={updatedFee} error={error} />
            ) : (
              <JsonResponse content={updatedFee || error} />
            )}
          </Box>
        </Box>
      )}
